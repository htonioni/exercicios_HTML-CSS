...
camposDoFormulario.forEach((barrinhaCampo) => {
    ...
    barrinhaCampo.addEventListener("invalid", evento => evento.preventDefault());
});

function verificaCampo(barrinhaCampo) {
    let mensagem = "";
    barrinhaCampo.setCustomValidity('');
    ...
    tiposDeErros.forEach(erro => {
        if(barrinhaCampo.validity[erro]){
            mensagem = mensagens[barrinhaCampo.name][erro]
            console.log(mensagem);
        }
    });
    const mensagemErro = barrinhaCampo.parentNode.querySelector('.mensagem-erro');
    const validadorDeInput = barrinhaCampo.checkValidity();
    if (!validadorDeInput){
